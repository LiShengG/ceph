...
// Truncating a file to an offset that falls *inside* an object and then growing
// it back (without rewriting) discards the tail of the partially truncated
// object.  blockdiff must still report that tail as changed, otherwise an
// incremental consumer (e.g. cephfs-mirror) keeps stale bytes there while the
// file size matches.  Regression test for the intra-object shrink defect in
...
TEST(LibCephFS, BlockDiffPartialTruncateThenSparseRegrow)
{
  ...
  // The whole region from the truncation point to EOF is now a hole and must be
  // reported.  for_each_file_blockdiff() erases every reported extent from the
  // expected set; if the partially truncated tail is dropped, the set is left
  // non-empty.
  interval_set<uint64_t> expected;
